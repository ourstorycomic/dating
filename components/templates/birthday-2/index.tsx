...
export default function Birthday2Diary() {
  ...
  return (
    <div className="relative w-full max-w-[400px] h-[800px] max-h-[90vh] bg-slate-900 overflow-hidden rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] mx-auto border-[12px] border-gray-800 text-white touch-none font-sans">
      <AnimatePresence mode="wait">
        {step === 1 && <Step1Alarm key="step1" onNext={handleNext} />}
        {step === 2 && <Step2FakeChat key="step2" messages={BIRTHDAY_DATA.fakeMessages} onNext={handleNext} />}
        {step === 3 && <Step3Delivery key="step3" onNext={handleNext} />}
        {step === 4 && <Step4Unbox key="step4" photos={BIRTHDAY_DATA.photos} onNext={handleNext} />}
        {step === 5 && <Step5Cake key="step5" onNext={handleNext} />}
        {step === 6 && <Step6Letter key="step6" letter={BIRTHDAY_DATA.letter} onNext={handleNext} />}
        {step === 7 && <Step7Climax key="step7" />}
      </AnimatePresence>
    </div>
  );
}
