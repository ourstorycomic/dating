...
export function Step2FakeChat({ messages, onNext, autoPlay = false, compact = false, config = {} }: { messages: string[]; onNext: () => void; autoPlay?: boolean; compact?: boolean; config?: Birthday2Config }) {
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 pt-12 pb-3 bg-gray-100/80 backdrop-blur-md border-b border-gray-200">
        <div className="flex items-center gap-2 text-blue-500">
          ...
          <span className="text-[17px]">{config?.Back || "Back"}</span>
        </div>
        <div className="flex flex-col items-center">
          <div className="w-10 h-10 bg-gray-300 rounded-full mb-1 flex items-center justify-center overflow-hidden">
            <span className="text-xl">👤</span>
          </div>
          <span className="text-[11px] font-semibold text-gray-800">{config?.Ngiygt || "Người ấy >"}</span>
        </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto flex flex-col gap-2 bg-white">
        <div className="text-center text-[11px] font-medium text-gray-400 my-4">{config?.Today705AM || "Today 7:05 AM"}</div>
        ...
        {visibleCount < messages.length && (
          ...
          >
            ...
            >
              ...
              {config?.Chmmnhnhctip || "Chạm màn hình để đọc tiếp"}
            </motion.div>
          </motion.div>
        )}
        ...
        <AnimatePresence>
          {visibleCount >= messages.length && (
            ...
            >
              ...
              >
                {config?.Bmgingay || "Bấm gửi ngay!"} <ArrowDown className="w-4 h-4" />
              </motion.div>
              <button
              ...
              >
                {config?.Hmnaysinhnhttm || "Hôm nay sinh nhật tớ mà 🥺"}
              </button>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="px-4 py-3 bg-gray-100/80 border-t border-gray-200 flex items-center gap-3">
        ...
        <div className="flex-1 bg-white border border-gray-300 rounded-full h-9 px-4 flex items-center">
          <span className="text-gray-400 text-[15px]">{config?.iMessage || "iMessage"}</span>
        </div>
      </div>
    </motion.div>
  );
