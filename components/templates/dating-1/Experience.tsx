...
const [stage, setStage] = useState<"question" | "success" | "location" | "datetime" | "food" | "drink" | "completion">("question");
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center overflow-hidden font-sans text-gray-800 ${compact ? "absolute inset-0 rounded-[2.5rem]" : fullScreen ? "relative min-h-[100dvh] w-full sm:rounded-2xl" : isBuilderPreview ? "relative h-full min-h-[820px] w-full sm:rounded-2xl" : "relative h-[1000px] min-h-[820px] max-h-[95vh] w-full sm:rounded-2xl border border-pink-100"}`}
      ...
      <AnimatePresence mode="wait">
        {stage === "question" && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="relative z-10 mx-auto w-[85%] sm:w-[90%] max-w-md min-h-[560px] max-h-[95%] overflow-y-auto [&::-webkit-scrollbar]:hidden rounded-[2rem] border border-white/60 bg-white/70 p-4 sm:p-6 text-center shadow-[0_12px_40px_rgba(255,192,203,0.5)] backdrop-blur-md flex flex-col items-center justify-center"
          >
            { questionImage ? (
