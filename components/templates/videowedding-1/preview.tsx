...
import { VideoWeddingOneComposition } from "./Composition";
...
export default function VideoWeddingOnePreview({
  customData,
  buyerName,
  recipientName,
  senderName,
  ...
  const enhancedCustomData = React.useMemo(() => {
    ...
    return { 
      ...
      text1: customData?.text1 || (senderName ? `${senderName} & ${recipientName}` : undefined),
      isCompact: compact
    };
  }, [customData, forceRandomMusic, senderName, recipientName, compact]);
  ...
  // If not active or noFrame/compact, we might just want to show a poster or basic preview
  return (
    <div className="w-full h-full flex items-center justify-center bg-[#1a0b0f] overflow-hidden rounded-[2.5rem]">
      <Player
        ref={playerRef}
        component={VideoWeddingOneComposition}
