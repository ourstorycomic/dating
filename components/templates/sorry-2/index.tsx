              ...
              {activeHole === i && (
                ...
                >
                  <div className="w-full h-full rounded-full bg-gradient-to-br from-pink-100 to-pink-300 border-4 border-slate-900 overflow-hidden relative shadow-[0_-5px_15px_rgba(0,0,0,0.5)]">
                    <MediaDisplay src={WHACK_DATA.avatar || "/assets/sad/el-gato.webp"} alt="avatar" className="w-full h-full object-cover pointer-events-none" />
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
...
function Step4Bandaged({ onNext, autoPlay, config }: { onNext: () => void; autoPlay: boolean; config: any }) {
  ...
  return (
    ...
    >
      ...
      >
        <MediaDisplay src={WHACK_DATA.photo2 || "/assets/sad/lots-of-tears-crying-hard.webp"} alt="avatar" className="w-full h-full object-cover filter brightness-90 sepia-[0.2]" />
...
// --- MAIN TEMPLATE ---
export default function Sorry2Template({ compact = false, autoPlay = false, hideNavigation = false, isBuilderPreview = false, config, generalAudioUrl, onStepChange }: { compact?: boolean; autoPlay?: boolean; hideNavigation?: boolean; isBuilderPreview?: boolean; config?: any; generalAudioUrl?: string ; onStepChange?: any}) {
  ...
  return (
    ...
    >
      ...
      {generalAudioUrl && <audio ref={audioRef} src={generalAudioUrl} loop autoPlay={autoPlay} muted={compact && !autoPlay && !isBuilderPreview} />}
        ...
        {step === 8 && (
          ...
          >
            ...
            <div className="bg-white/40 p-6 rounded-3xl backdrop-blur-md border border-white/60 shadow-2xl text-slate-800">
              ...
              <MediaDisplay src={WHACK_DATA.photo3 || "/assets/happy/cute-love.webp"} className="w-32 h-32 object-cover mx-auto animate-bounce mt-4 drop-shadow-xl rounded-2xl border-4 border-white" alt="cute" />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
